...
class License
{
    ...
    public function register_hooks()
    {
        ...
        add_action('woocommerce_checkout_after_order_review', [$this, 'add_sales_rep_field_to_checkout']);
     ...
     /**
     * Add sales rep select field to checkout page
     */
   public function add_sales_rep_field_to_checkout()
{
    try {
        // Get sales reps from API
        $sales_reps = $this->get_sales_reps();
        
        if (empty($sales_reps)) {
            return; // Don't show field if no sales reps available
        }

        // Get default sales rep from settings
        $default_sales_rep_id = $this->existing_settings['sales_rep_id'] ?? '';

        // Prepare options array
        $options = ['' => __('No Sales Rep', 'flourish-woocommerce')];
        $default_rep_name = '';
        ...
        foreach ($sales_reps as $rep) {
            ...
            if ($rep_id) {
                $options[$rep_id] = $rep_name;
                
                // Store the default rep name for display
                if ($rep_id == $default_sales_rep_id) {
                    $default_rep_name = $rep_name;
                }
            }
        }

        // Set the default value - use the default sales rep from settings
        $selected_value = WC()->checkout->get_value('sales_rep_id') ?: $default_sales_rep_id;
        ...
        woocommerce_form_field('sales_rep_id', array(
            ...
            'label'       => __('Sales Representative', 'flourish-woocommerce'),
            'placeholder' => __('Select your sales representative', 'flourish-woocommerce'),
            'required'    => false, // Set to true if required
            'options'     => $options,
            'description' => $default_rep_name ? 
                sprintf(__('Default: %s. You can change if needed.', 'flourish-woocommerce'), $default_rep_name) :
                __('Please select your assigned sales representative.', 'flourish-woocommerce'),
        ), $selected_value);
    ...
    /**
     * Get sales reps from Flourish API
     */
    ...
    public function save_order_meta($order_id)
    {
        ...
        if (!empty($_POST['sales_rep_id'])) {
            ...
            // Also get and save the sales rep name for easy display
            $sales_reps = $this->get_sales_reps();
            foreach ($sales_reps as $rep) {
                if (isset($rep['id']) && $rep['id'] == $sales_rep_id) {
                    $rep_name =(isset($rep['first_name'], $rep['last_name']) ? 
                                $rep['first_name'] . ' ' . $rep['last_name'] : 
                                'Sales Rep');
                    update_post_meta($order_id, '_sales_rep_name', $rep_name);
                    break;
                }
            }
        }
